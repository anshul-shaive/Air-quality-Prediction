...
public class WaqiObject extends SugarRecord {
    ...
    public String getMinTemp() {
        ...
        if (this.globalObject != null) {
            ...
            if(this.globalObject.getRxs().getObs().get(index).getMsg().getForecast().getAqi().size() > 0) {
                minTemp = String.format("Min : %d", this.globalObject.getRxs().getObs().get(index).getMsg().getForecast().getAqi().get(0).getV().get(0));
            }
        }
        return minTemp;
    }
    ...
    public String getMaxTemp() {
        ...
        if (this.globalObject != null) {
            int index = globalObject.getRxs().getObs().size() - 1;
            if(this.globalObject.getRxs().getObs().get(index).getMsg().getForecast().getAqi().size() > 0) {
                maxTemp = String.format("Max : %d", this.globalObject.getRxs().getObs().get(index).getMsg().getForecast().getAqi().get(0).getV().get(1));
            }
        }
        return maxTemp;
    }
